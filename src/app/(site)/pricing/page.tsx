...
export default async function PricingPage() {
  ...
  const tiers = sanityPricing.length > 0
    ? sanityPricing.map((t) => ({
        ...t,
        features:   t.features   ?? [],
    ...
    <>
      ...
      <MotionSection className="bg-background">
          ...
          <p className="text-center text-sm text-muted-foreground mb-10">
            All prices in <strong>CAD</strong>. Custom quotes available for commercial and large-scale projects.
          </p>

          <Stagger className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:items-start">
            {tiers.map((tier: any) => {
              const styles = COLOR_STYLES[tier.color as keyof typeof COLOR_STYLES] ?? COLOR_STYLES.green;
              return (
                <StaggerItem key={tier.id ?? tier.name}>
                <div
                  className={cn(
                    "relative flex h-full flex-col rounded-2xl border bg-card shadow-card overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-card-hover",
          ...
          <div className="mt-12 rounded-2xl border border-blue-200 bg-blue-50 p-8 text-center dark:border-blue-800 dark:bg-blue-950/20">
            ...
            <p className="text-muted-foreground mb-6 max-w-xl mx-auto">
              Our commercial waste management plans are tailored to your business size,
              waste volume, and pickup frequency. Pricing scales with your needs.
            </p>
      ...
      <MotionSection className="bg-section-alt" containerClassName="max-w-3xl mx-auto text-center">
          ...
          <Stagger className="grid gap-4 sm:grid-cols-3 mt-8">
            {[
              { emoji: "📱", title: "SMS Reminders",      desc: "Day-before pickup notifications so you're never caught off guard." },
              { emoji: "🌱", title: "Eco Disposal",        desc: "We sort, recycle, and minimize landfill impact on every pickup." },
              { emoji: "🛡️",  title: "Fully Insured",       desc: "Full liability coverage on every crew member and job." },
            ].map(({ emoji, title, desc }) => (
              <StaggerItem key={title}>
                <div className="rounded-xl border border-border bg-card p-5 shadow-card text-center transition-all duration-300 hover:-translate-y-0.5 hover:shadow-card-hover">
