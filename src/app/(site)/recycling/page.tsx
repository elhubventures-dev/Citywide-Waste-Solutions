...
export const metadata: Metadata = {
  ...
  description: `A complete guide to recycling in Ontario — what's accepted, sorting tips, pickup schedules, and how ${BUSINESS.name} handles your recyclables responsibly.`,
...
const RECYCLABLE = [
  {
    ...
    items: ["Newspapers", "Flattened cardboard boxes", "Office paper", "Magazines", "Paper bags"],
    icon: "📄",
  },
  {
    label: "Plastic Bottles & Jugs",
    items: [
      "Water/juice bottles",
      "Milk jugs",
      "Shampoo bottles",
      "Detergent containers",
      "Yogurt containers (#1–#7)",
    ],
    icon: "♻️",
  },
  {
    label: "Glass Containers",
    items: ["Clear, green & brown bottles", "Food jars", "Beverage bottles (empty & rinsed)"],
    icon: "🫙",
  },
  {
    label: "Metal Cans",
    items: ["Aluminum cans", "Steel/tin food cans", "Empty aerosol cans", "Foil pans (clean)"],
    icon: "🥫",
  },
  {
...
const TIPS = [
  {
    ...
    desc: "Food residue contaminates entire batches. A quick rinse is all it takes.",
  },
  {
    emoji: "📦",
    title: "Flatten cardboard boxes",
    desc: "Flattened boxes take up less space and are easier to sort at facilities.",
  },
  {
    emoji: "🎁",
    title: "Remove non-paper materials",
    desc: "Bubble wrap, tape, and plastic windows on envelopes must be removed first.",
  },
  {
    emoji: "🏷️",
    title: "Labels are OK to leave on",
    desc: "Modern sorting facilities handle labels — no need to peel them off.",
  },
  {
    emoji: "🪣",
    title: "Keep lids on bottles",
    desc: "Screw caps back on after rinsing — they're recyclable and easier to process.",
  },
  {
    emoji: "📵",
    title: "No plastic bags in recycling bins",
    desc: "Bags clog sorting machinery. Return them to grocery stores instead.",
  },
];
    ...
    <>
      ...
      <MotionSection className="bg-background" aria-labelledby="schedule-heading">
        ...
        <p className="mt-4 text-center text-sm text-muted-foreground">
          * Schedules subject to change on statutory holidays. SMS reminders sent automatically when
          you sign up.
        </p>
      </MotionSection>
      ...
      <MotionSection className="bg-background">
        ...
        <div className="mx-auto grid max-w-4xl gap-6 sm:grid-cols-2">
          {SPECIALIZED_SOLUTIONS.filter(
            (s) => s.title.includes("Wood") || s.title.includes("Paper")
          ).map((item) => (
            <div
              key={item.title}
      ...
      <MotionSection className="bg-section-alt">
        <div className="grid gap-10 lg:grid-cols-2">
          {/* PDF Download card */}
          <div className="flex flex-col gap-5 rounded-2xl bg-hero-gradient p-8 text-white">
            <div className="text-4xl">📥</div>
            <div>
              <h2 className="mb-2 text-2xl font-bold">Download the Full Recycling Guide</h2>
            ...
            <ul className="space-y-2 text-sm text-white/80">
              {[
                "Complete materials A–Z",
                "Ontario-specific regulations",
                "Sorting illustrations",
                "Specialty disposal locations",
              ].map((f) => (
            ...
            <a
              ...
              download
              className="inline-flex w-fit items-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-bold text-green-700 transition-colors hover:bg-green-50"
            >
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-card sm:p-8">
            <h2 className="mb-1 text-xl font-bold text-foreground">Start Your Recycling Service</h2>
