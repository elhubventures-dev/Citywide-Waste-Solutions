...
export async function generateMetadata({
  ...
  const sanityService = await sanityFetch<any>(
    serviceBySlugQuery, { slug: params.slug }, ["services"]
  ).catch(() => null);
  ...
  const description  = sanityService?.seo?.metaDescription ?? sanityService?.description ?? staticService?.description;

  if (!title) return { title: "Service Not Found" };

  return {
    title:       `${title} in Ontario`,
    description: description ?? `Professional ${title} services across Ontario. Get your free quote today.`,
      ...
      {faqs.map((faq) => (
        ...
        >
          <summary className="flex cursor-pointer items-center justify-between gap-4 px-5 py-4 font-semibold text-foreground hover:text-green-600 transition-colors list-none">
            {faq.question}
            <ChevronDown className="h-4 w-4 shrink-0 transition-transform duration-300 group-open:rotate-180 text-muted-foreground" />
          </summary>
...
export default async function ServicePage({
  ...
  const serviceSchema = {
    ...
    areaServed: ["Vaughan","Toronto","Brampton","Mississauga","Courtice"],
  ...
  return (
    <>
      ...
      <MotionSection className="bg-background">
          <div className="grid gap-16 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1">
              ...
              >
                <h2 className="text-xl font-bold text-foreground mb-1">Get Your Free Quote</h2>
      ...
      <MotionSection className="bg-section-alt">
          <h2 className="text-2xl font-bold text-foreground mb-8">Other Services We Offer</h2>
          <Stagger className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {SERVICES.filter((s) => s.slug !== params.slug).map((s) => {
              ...
              return (
                ...
                >
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-green-100 text-green-600 group-hover:bg-green-500 group-hover:text-white transition-colors dark:bg-green-950/40">
                    <S className="h-4 w-4" />
                  </div>
                  <div>
                  ...
                  <ArrowRight className="ml-auto h-4 w-4 self-center text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
                </Link>
                </StaggerItem>
              );
            })}
